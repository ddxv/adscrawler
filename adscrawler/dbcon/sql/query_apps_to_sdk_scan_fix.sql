WITH latest_version_codes AS (
	...
	FROM
		version_codes vc_1
    LEFT JOIN store_apps sa ON vc_1.store_app = sa.id
	WHERE
		vc_1.version_code != '-1' and sa.store = :store AND vc_1.created_at < current_date - INTERVAL '3 days'
	ORDER BY
		vc_1.store_app,
...
FROM results lvc
LEFT JOIN adtech.app_sdk_strings s ON s.store_app = lvc.store_app

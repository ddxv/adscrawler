    ...
    WITH countries_to_crawl AS (
        SELECT
            ...
            cc.priority
        FROM
            public.crawl_scenario_country_config cc
        JOIN public.countries c ON
            cc.country_id = c.id
        JOIN public.crawl_scenarios s ON
            cc.scenario_id = s.id
        WHERE
            s.name = 'app_details'
            AND cc.enabled = TRUE
    ...
    CROSS JOIN countries_to_crawl ctc
    LEFT JOIN latest_crawls lc ON 
        sa.id = lc.store_app

...
CREATE MATERIALIZED VIEW public.apps_new_monthly AS
 WITH rankedapps AS (
         SELECT sa.id,
          ...
          WHERE ((sa.release_date >= (CURRENT_DATE - '30 days'::interval)) AND (sa.crawl_result = 1))
        )
 SELECT id,
    developer,
    name,
...
CREATE MATERIALIZED VIEW public.apps_new_weekly AS
 WITH rankedapps AS (
         SELECT sa.id,
           ...
           FROM (public.store_apps sa
             JOIN public.category_mapping cm ON (((sa.category)::text = (cm.original_category)::text)))
          WHERE ((sa.release_date >= (CURRENT_DATE - '7 days'::interval)) AND (sa.crawl_result = 1))
        )
 SELECT id,
    developer,
    name,
...
CREATE MATERIALIZED VIEW public.apps_new_yearly AS
 WITH rankedapps AS (
         SELECT sa.id,
           ...
           FROM (public.store_apps sa
             JOIN public.category_mapping cm ON (((sa.category)::text = (cm.original_category)::text)))
          WHERE ((sa.release_date >= (CURRENT_DATE - '365 days'::interval)) AND (sa.crawl_result = 1))
        )
 SELECT id,
    developer,
    name,
 ...
 WITH developer_domain_ids AS (
         SELECT DISTINCT pd_1.id AS domain_id
           FROM (((public.app_urls_map aum_1
             LEFT JOIN public.pub_domains pd_1 ON ((aum_1.pub_domain = pd_1.id)))
...
CREATE MATERIALIZED VIEW public.total_count_overview AS
 WITH app_count AS (
         ...
         SELECT count(DISTINCT pd.url) AS appads_urls,
            count(DISTINCT
                CASE
            ...
            count(DISTINCT
                CASE
                    WHEN ((pd.crawl_result = 1) AND (pd.updated_at >= (CURRENT_DATE - '7 days'::interval))) THEN pd.url
                    ELSE NULL::character varying
                END) AS appads_weekly_success_urls,
            count(DISTINCT
                CASE
                    WHEN (pd.updated_at >= (CURRENT_DATE - '7 days'::interval)) THEN pd.url
                    ELSE NULL::character varying
                END) AS appads_weekly_urls
           FROM public.pub_domains pd
        )
...
CREATE UNIQUE INDEX idx_apps_new_weekly ON public.apps_new_weekly USING btree (store, mapped_category, store_id);
...
ALTER TABLE ONLY public.app_ads_map
    ADD CONSTRAINT app_ads_map_fk_1 FOREIGN KEY (pub_domain) REFERENCES public.pub_domains(id);
...
ALTER TABLE ONLY public.app_ads_entrys
    ADD CONSTRAINT app_ads_txt_fk FOREIGN KEY (ad_domain) REFERENCES public.ad_domains(id);
...
ALTER TABLE ONLY public.app_urls_map
    ADD CONSTRAINT app_urls_fk_1 FOREIGN KEY (pub_domain) REFERENCES public.pub_domains(id);

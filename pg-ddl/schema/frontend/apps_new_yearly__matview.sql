...
\restrict dZKOI6L3PgdDZzIesgOhUWXtY8ej6demAXyXgHXWPVCsH1TGeKehDeueEdNmWSe
...
WITH rankedapps AS (
    ...
    WHERE
        (
            (sa_1.release_date >= (current_date - '365 days'::interval))
            AND (sa_1.created_at >= (current_date - '380 days'::interval))
...
CREATE UNIQUE INDEX idx_apps_fr_new_yearly ON frontend.apps_new_yearly USING btree (
...
CREATE UNIQUE INDEX idx_apps_new_yearly ON frontend.apps_new_yearly USING btree (
    store, app_category, store_id
);
...
\unrestrict dZKOI6L3PgdDZzIesgOhUWXtY8ej6demAXyXgHXWPVCsH1TGeKehDeueEdNmWSe

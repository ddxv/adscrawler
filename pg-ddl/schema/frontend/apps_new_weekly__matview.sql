...
\restrict Mr91OikOkhObfdVYU0nleGe1zdaIyaZr19EKDA7eOTLQHL3qF8KeUXYtc37Kw1T
...
WITH rankedapps AS (
    SELECT
        ...
        row_number()
            OVER (
                PARTITION BY sa_1.store, sa_1.category
                ORDER BY
                    sa_1.installs DESC NULLS LAST,
                    sa_1.rating_count DESC NULLS LAST
            )
            AS rn
    FROM frontend.store_apps_overview AS sa_1
    WHERE
        (
            (sa_1.release_date >= (current_date - '7 days'::interval))
            AND (sa_1.created_at >= (current_date - '11 days'::interval))
            AND (sa_1.crawl_result = 1)
        )
)
SELECT
    ra.id,
...
FROM (
    rankedapps AS ra
    LEFT JOIN public.store_apps AS sa ON ((ra.id = sa.id))
)
WHERE (ra.rn <= 100)
WITH NO DATA;
...
CREATE UNIQUE INDEX idx_apps_fr_new_weekly ON frontend.apps_new_weekly USING btree (
    store, app_category, store_id
);
...
CREATE UNIQUE INDEX idx_apps_new_weekly ON frontend.apps_new_weekly USING btree (
    store, app_category, store_id
);
...
CREATE UNIQUE INDEX idx_apps_new_weekly_f ON frontend.apps_new_weekly USING btree (
    store, app_category, store_id
);
...
\unrestrict Mr91OikOkhObfdVYU0nleGe1zdaIyaZr19EKDA7eOTLQHL3qF8KeUXYtc37Kw1T

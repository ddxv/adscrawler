...
\restrict 6mS6WNF3HOY3xazD5m3Js41g7EmGJIIQg55zvcJxVO0iDEvFPN4VywDtPHFzfhf
...
WITH rankedapps AS (
    ...
    WHERE
        (
            (sa_1.release_date >= (current_date - '30 days'::interval))
            AND (sa_1.created_at >= (current_date - '45 days'::interval))
...
CREATE UNIQUE INDEX idx_apps_fr_new_monthly ON frontend.apps_new_monthly USING btree (
...
CREATE UNIQUE INDEX idx_apps_new_monthly ON frontend.apps_new_monthly USING btree (
    store, app_category, store_id
);
...
\unrestrict 6mS6WNF3HOY3xazD5m3Js41g7EmGJIIQg55zvcJxVO0iDEvFPN4VywDtPHFzfhf
